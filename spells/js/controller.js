...
Vue.component('card', {
	...
	computed: {
		...
		secondCardBack: function () {
			return this.toogleCardBack;
		},
		typeClass: function(){
		...
		textSizeStyle: function(){
			return this.textSize? "font-size: "+this.textSize+"px; line-height: "+(Math.ceil(this.textSize+2))+"px": "";
		},
	...
	updated: function() {
	/* ================================================================== */
		const textContainerHeight = this.$refs.itemTextContainer?.getBoundingClientRect().height;
		const textHeight = this.$refs.itemText?.getBoundingClientRect().height;

        let lineSize = Math.ceil(this.textSize + 2);
        let lines = Math.floor(textContainerHeight / lineSize);
        let maxHeight = lines * lineSize;

		this.$refs.additionalTextContainer?.setAttribute('style', `margin-top: ${-1*(maxHeight)-1}px`);

		if (!this.isOverflow) {
		    if (textHeight > textContainerHeight) {
                this.$refs.itemTextContainer?.setAttribute('style', `max-height: ${maxHeight+1}px`);

                this.isOverflow = true;
                this.cardWidth = 482;
				//this.$refs.wholeCardBack.classList.add("wide");
                this.$forceUpdate();
			}
		}
		...
		if (textHeight <= textContainerHeight) {
            if (this.isOverflow) {
                this.cardWidth = 240;
				//this.$refs.wholeCardBack.classList.remove("wide");
                this.$forceUpdate();
            }
	...
	methods: {
		...
		onTextMin: function(){
			this.textSize=this.textSize-0.25;
		},
		onTextMax: function(){
			this.textSize=this.textSize+0.25;
		},
		...
		setCardsBack: function(bToogle) {
		    if (bToogle) {
                const wholeCardContainer = this.$refs.wholeCardContainer.setAttribute('style', `display: none;`);
				const wholeCardBack = this.$refs.wholeCardBack.setAttribute('style', `display: inline-block;`);
				if (this.cardWidth > 400) {
					const wholeCardBackSec = this.$refs.wholeCardBackSec.setAttribute('style', `display: inline-block;`);
				}
            }
            else {
            	const wholeCardContainer = this.$refs.wholeCardContainer.setAttribute('style', `display: flex;`);
				const wholeCardBack = this.$refs.wholeCardBack.setAttribute('style', `display: none;`);
				const wholeCardBackSec = this.$refs.wholeCardBackSec.setAttribute('style', `display: none;`);
            }
		}
	...
	template:
	`<div :class="[mainClass, viewClass, colorClass]" @click.ctrl="select" v-on:dblclick.stop="select" ref="cardContainer" :style="cardWidthStyle">
     	<div :class='[ItemCard, selectedClass]' v-if="cardView" >
     		...
     		<div class="container" ref="wholeCardContainer">
     			<div class="content">
     				<span v-show="locked" class="bUnlockItem" title="Открепить обратно" @click.stop="unlock">
     					<i class="fa fa-unlock-alt" aria-hidden="true"/>
     				</span>
     				<span v-show="!locked" class="bLockItem" title="Закрепить заклинание (не будут действовать фильтры)" @click.stop="lock">
     					<i class="fa fa-lock" aria-hidden="true"/>
     				</span>
     				<span class="bHideItem" title="Скрыть заклинание (будет внизу панели фильтров)" @click.stop="hide">
     					<i class="fa fa-eye-slash" aria-hidden="true"/>
     				</span>
     				<h1 :title="tooltip" :contenteditable="editable">
     					<div style="padding-top: 1px;">{{name}} {{ritualMark}}</div>
     					<div class="circle">{{level}}</div>
     				</h1>
     				<div class="row">
     				...
     				<div class="text" ref="itemTextContainer">
     					<div v-html="preparedText" :style="textSizeStyle" ref="itemText" :contenteditable="editable"/>
     				</div>
     				...
     				<!--b class="class">{{spellClasses}}</b-->
     				<div class="footer">
     					<b class="school">{{school}}</b>
     					<span class="school" :title="srcTitle">{{src}}</span>
     				</div>
     			</div>
     			<div class="content additional" v-if="this.isOverflow">
     				<div class="text" ref="additionalTextContainer">
     					<div v-html="preparedText" :style="textSizeStyle" :contenteditable="editable"/>
     				</div>
     			</div>
     		</div>
     	</div>
     	...
     	<div class="inner" v-if="!cardView">
     		<span v-show="locked" class="bUnlockItem noprint" title="Открепить обратно" @click.stop="unlock">
     			<i class="fa fa-unlock-alt" aria-hidden="true"/>
     		</span>
     		<span v-show="!locked" class="bLockItem noprint" title="Закрепить заклинание (не будут действовать фильтры)" @click.stop="lock">
     			<i class="fa fa-lock" aria-hidden="true"/>
     		</span>
     		<span class="bHideItem noprint" title="Скрыть заклинание (будет внизу панели фильтров)" @click.stop="hide">
     			<i class="fa fa-eye-slash" aria-hidden="true"/>
     		</span>
     		<div class="flex">
     			...
     			<div class="flex secondal">
     				<div class="column thirdal">
     					<div class="cvasi_row">
     						<div class="subtitle" :contenteditable="editable">{{castingTimeTitle}}</div>
     						<div>{{castingTime}}</div>
     					</div>
     					<div class="cvasi_row">
     						<div class="subtitle" :contenteditable="editable">{{rangeTitle}}</div>
     						<div>{{range}}</div>
     					</div>
     				</div>
     				<div class="column thirdal">
     					<div class="cvasi_row">
     						<div class="subtitle" :contenteditable="editable">{{componentsTitle}}</div>
     						<div>{{components}} {{materials?"*":""}}</div>
     					</div>
     					<div class="cvasi_row">
     						<div class="subtitle" :contenteditable="editable">{{durationTitle}}</div>
     						<div>{{duration}}</div>
     					</div>
     				</div>
     			</div>
     		...
     		<div class="text" v-html="preparedText" :contenteditable="editable" ref="itemText"/>
  ...
  var app = new Vue({
    ...
    data: {
		...
		computed: {
				...
				for (var key in this.aCastingTime){
					if(this.aCastingTime[key].visible !== false){
						a.push({
							...
							title: this.aCastingTime[key].text.ru.title,
				...
				for (var key in this.aSchools){
					if(this.aSchools[key].visible !== false){
						...
						a.push({
							...
							title: this.aSchools[key].text.ru.title,
			...
			aClassList: function(){
				...
				for (let key in this.oClassSpells) {
					...
					let sTitle = (this.oClassSpells[key].title.ru.text || this.oClassSpells[key].title.ru);
					aSclasses.push({
						key: key,
						title: sTitle
					});
			...
			sClassTitle: function(){
			    ...
			    const { Bard: bard
			          ...
			          , Sorcerer: sorcerer
			          , Warlock: warlock
			          , Wizard: wizard } = this.oClassSpells;

                spells = bard.spells;
                for (var i in spells) {
                    if (classList[spells[i].toLowerCase()])
                        classList[spells[i].toLowerCase()] = classList[spells[i].toLowerCase()] + " bard";
                    else
                        classList[spells[i].toLowerCase()] = "bard";
                }

                spells = cleric.spells;
                for (var i in spells) {
                    if (classList[spells[i].toLowerCase()])
                        classList[spells[i].toLowerCase()] = classList[spells[i].toLowerCase()] + " cleric";
                    else
                        classList[spells[i].toLowerCase()] = "cleric";
                }

                spells = druid.spells;
                for (var i in spells) {
                    if (classList[spells[i].toLowerCase()])
                        classList[spells[i].toLowerCase()] = classList[spells[i].toLowerCase()] + " druid";
                    else
                        classList[spells[i].toLowerCase()] = "druid";
                }

                spells = paladin.spells;
                for (var i in spells) {
                    if (classList[spells[i].toLowerCase()])
                        classList[spells[i].toLowerCase()] = classList[spells[i].toLowerCase()] + " paladin";
                    else
                        classList[spells[i].toLowerCase()] = "paladin";
                }

                spells = ranger.spells;
                for (var i in spells) {
                    if (classList[spells[i].toLowerCase()])
                        classList[spells[i].toLowerCase()] = classList[spells[i].toLowerCase()] + " ranger";
                    else
                        classList[spells[i].toLowerCase()] = "ranger";
                }

                spells = sorcerer.spells;
                for (var i in spells) {
                    if (classList[spells[i].toLowerCase()])
                        classList[spells[i].toLowerCase()] = classList[spells[i].toLowerCase()] + " sorcerer";
                    else
                        classList[spells[i].toLowerCase()] = "sorcerer";
                }

                spells = warlock.spells;
                for (var i in spells) {
                    if (classList[spells[i].toLowerCase()])
                        classList[spells[i].toLowerCase()] = classList[spells[i].toLowerCase()] + " warlock";
                    else
                        classList[spells[i].toLowerCase()] = "warlock";
                }

                spells = wizard.spells;
                for (var i in spells) {
                    if (classList[spells[i].toLowerCase()])
                        classList[spells[i].toLowerCase()] = classList[spells[i].toLowerCase()] + " wizard";
                    else
                        classList[spells[i].toLowerCase()] = "wizard";
                }
                //console.log(classList);
			    /*bard.spells.forEach(function(spell){
			        if (classList.has(spell)) {
			            classList.set(spell, classList.get(spell) + " " + "bard");
			        }
			        else {
			            classList.set(spell, classList.get(spell) + " " + "bard");
			        }
			    ...
			    classSpells.forEach(function(oClass){
			        console.log(sName, oClass.spells[sName], oClass.title["ru"]);
			        if (oClass.spells[sName])
			            classList = classList + " " + oClass.title["ru"];
			    });*/
				return classList;
			},
			...
			aItemsList: function(){
				let aFiltered = /*/this.aItems/**/ this.aClassSpells.filter(function(oItem){
					return (
						...
						this.aSchoolSelected.indexOf(oItem.ru.school.toLowerCase().trim())>-1 /**/&&
						(
				...
				return aFiltered.map(function(oItem){
					...
					let spellClassesVar = this.sClassTitle[oItem.en.name.toLowerCase()];
					...
					let o={
						...
						"className": spellClassesVar,
						"source": sSrc, /*this.aSources[oItem.en.source].text[this.sLang].title*/
						"school": this.aSchools[oItem.ru.school.trim()].text[this.sLang].title,
						"level": oItem.en.level, //oLevelsText[oItem.en.level]? oLevelsText[oItem.en.level].text[this.sLang].title : oItem.en.level + " " + oLevelsText.units[this.sLang].title,
		...
		methods: {
			...
			onToogleCardBackPress: function() {
				this.bToogleCardBack = !this.bToogleCardBack;
                ...
                aCards.forEach(function(oCard){
                    oCard.setCardsBack(this.bToogleCardBack);
                }.bind(this));
			},
